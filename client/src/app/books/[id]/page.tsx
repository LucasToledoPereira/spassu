...
export default function EditBook({ params: { id } }: any) {
  const { put, success, error } = usePut<BaseResponse<BooksResponse>>(`${process.env.NEXT_PUBLIC_API}books/${id}`);
  ...
  const { response, loading } = useGet<BaseResponse<BooksResponse>>(`${process.env.NEXT_PUBLIC_API}books/${id}`, {eager: true, cache: false})
  ...
  const {
  ...
  } = useForm<BooksInputs>(
    {
      values: {
        ...
        edition: response?.data.edition || 0,
        authors: response?.data.authors?.map(a => `${a.id}`) || [],
        subjects: response?.data.subjects?.map(s => `${s.id}`) || []
      }
    }
  )

  const route = useRouter();
  const onSubmit: SubmitHandler<BooksInputs> = (data) => put({
    year: data.year,
    value: +data.value,
